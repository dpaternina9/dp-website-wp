 ...
 * names this one explicitly — there is no default path.
 *
 * It delegates to the capability that governs the object itself rather than
 * inventing a capability of its own: if you may edit the post, you may edit the
 ...
 *
 * There is no term counterpart. `Meta` registers no term meta — a series' deck is
 * its core `description` — and an unreachable callback kept against the day a
 * term field returns is a callback nothing tests.
 *
 * It answers for `$user_id` rather than for the current user. WordPress passes
